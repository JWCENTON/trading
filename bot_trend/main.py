    ...
    *,
    ...
    if closed:
        logging.info(
            ...
            enriched_reason, float(exit_price), str(open_time)
        )
        emit_strategy_event(
...
def load_runtime_params():
    ...
    logging.info(
        ...
        bool(PROFIT_LOCK_CONFIG.enabled),
        float(PROFIT_LOCK_CONFIG.arm_pct),
        float(PROFIT_LOCK_CONFIG.floor_pct),
        float(PROFIT_LOCK_CONFIG.trail_drop_pct),
        float(PROFIT_LOCK_CONFIG.min_age_minutes),
...
def _execute_and_record_after_paper_exit_preflight(
    ...
    *,
    ...
    if not is_exit:
        outcome = normalize_entry_execution_outcome(
            resp,
            requested_qty=qty_btc,
            client_order_id=client_order_id,
        )
        status_raw = str(outcome.exchange_status or "").upper()
        executed_f = float(outcome.executed_qty or 0.0)
        order_id = outcome.order_id
    ...
    emit_strategy_event(
        ...
        reason=(
            "OK" if live_ok
            else "ORDER_ACCEPTED_PENDING_FILL" if order_accepted
            else "ORDER_REJECTED" if not is_exit
            else "ACK_NO_FILL"
        ),
        price=price,
        candle_open_time=candle_open_time,
    ...
    if (
        ...
        try:
            ...
            pos_id_new = None
            logging.exception(
                "TREND live entry fill position write failed symbol=%s client_order_id=%s order_id=%s",
    ...
    return {
        ...
        "live_ok": live_ok,
        "blocked_reason": (
            None if outcome.executed
            else "ORDER_ACCEPTED_PENDING_FILL" if outcome.order_accepted
            else "ORDER_REJECTED"
        ),
        "client_order_id": outcome.client_order_id,
        "resp": raw,
        ...
        if has_position:
            ...
            if pos_entry_time is not None:
                ...
                if profit_lock_decision.triggered:
                    ...
                    reason_profit_lock = (
                        f"TREND {exit_kind} {profit_lock_decision.trigger_type} "
...
def run_long_horizon_l3_owner_cycle():
    ...
    res = execute_and_record(
        ...
        allow_meta={"authority": "LOCAL_PAPER_L3"}, is_exit=True, pos_id=int(pos_id),
    )
    if res["ledger_ok"] and "position_close_succeeded" not in res:
        close_position(exit_price=float(decision.mark_price), reason=L3_TARGET_EXIT_REASON,
