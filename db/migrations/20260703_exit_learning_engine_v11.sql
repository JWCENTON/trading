...
BEGIN
  ...
  WITH base AS (
    SELECT
      ...
      AVG(
        CASE
          WHEN mfe_pct IS NOT NULL
           AND mfe_pct > 0
           AND exit_pct IS NOT NULL
           AND exit_pct > 0
          THEN LEAST(GREATEST(exit_pct / NULLIF(mfe_pct, 0), 0), 1)
          ELSE NULL
        END
      ) AS avg_mfe_capture_ratio,

      AVG(
        CASE
          WHEN mfe_pct IS NOT NULL
           AND mfe_pct > 0
           AND giveback_pct IS NOT NULL
           AND giveback_pct >= 0
          THEN LEAST(GREATEST(giveback_pct / NULLIF(mfe_pct, 0), 0), 1)
          ELSE NULL
        END
      ) AS avg_giveback_ratio
    FROM exit_trace_v2
    ...
    SELECT
      *,
      ...
      CASE
        ...
        THEN GREATEST(
          0,
          COALESCE(avg_giveback_pct, 0) * trades * 0.01
        )
        ELSE 0
      ...
      CASE
        ...
        WHEN exit_decision_class_v2 IN (
          'LOSS_AFTER_AVAILABLE_PROFIT_REVIEW',
          'STOP_AFTER_PROFIT_AVAILABLE_REVIEW'
        ) THEN 'Potential recovery from protecting available profit earlier'
        ELSE 'No recoverable estimate for this class'
      END AS recoverable_reason
    FROM base
  )
  SELECT
    strategy,
...
CREATE OR REPLACE VIEW v_exit_learning_top_recoverable_v1 AS
SELECT *
