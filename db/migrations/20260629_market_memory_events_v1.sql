...
CREATE TABLE IF NOT EXISTS market_memory_events (
...
CREATE INDEX IF NOT EXISTS ix_market_memory_events_active
ON market_memory_events(expires_at DESC, score DESC);

CREATE INDEX IF NOT EXISTS ix_market_memory_events_symbol_interval
ON market_memory_events(symbol, interval, observed_at DESC);

CREATE INDEX IF NOT EXISTS ix_market_memory_events_type
ON market_memory_events(event_type, observed_at DESC);
...
BEGIN
  ...
  INSERT INTO market_memory_events (
    symbol, interval, event_type, event_key,
    score, direction, regime, confidence,
    window_label, observed_at, expires_at,
    reason, payload
  )
  WITH mm AS (
    SELECT *
    FROM market_memory_snapshot
    WHERE window_label IN ('15m','1h','6h','24h')
  ),
  base AS (
    SELECT
      mm.*,
  ...
  events AS (
    -- strong short-window upside impulse
    SELECT
      symbol,
      interval,
    ...
    -- upside reversal after longer negative context
    SELECT
      s.symbol,
      s.interval,
    ...
    -- breakout up: new strong range with positive close direction
    SELECT
      symbol,
      interval,
  ...
  SELECT
    ...
    interval,
    event_type,
    md5(symbol || '|' || interval || '|' || event_type || '|' || window_label || '|' || date_trunc('minute', observed_at)::TEXT) AS event_key,
    score,
    direction,
    regime,
    confidence,
