...
CREATE INDEX IF NOT EXISTS ix_lfa_positions_closed_exit_time
ON positions(status, exit_time DESC);

CREATE INDEX IF NOT EXISTS ix_lfa_entry_trace_slot_time
ON entry_trace_events(symbol, "interval", strategy, created_at DESC);

CREATE INDEX IF NOT EXISTS ix_lfa_strategy_events_slot_time
ON strategy_events(symbol, "interval", strategy, created_at DESC);

CREATE INDEX IF NOT EXISTS ix_lfa_slot_brain_slot_window_time
ON slot_brain_snapshot(symbol, "interval", strategy, window_label, calculated_at DESC);
...
DROP VIEW IF EXISTS v_learning_orc_context_compat_v1;

DO $$
BEGIN
  IF to_regclass('public.v_orc_integration_v2_best') IS NOT NULL THEN
    EXECUTE $v$
      CREATE VIEW v_learning_orc_context_compat_v1 AS
      SELECT
        symbol, "interval", strategy,
        orc_final_score_v2,
        eligible_v63,
        orc_v7_ready,
        context_v2_ready_now,
        v63_reason,
        readiness_reason
      FROM v_orc_integration_v2_best
    $v$;
  ELSIF to_regclass('public.v_orc_candidate_context_best_v1') IS NOT NULL THEN
    EXECUTE $v$
      CREATE VIEW v_learning_orc_context_compat_v1 AS
      SELECT
        symbol, "interval", strategy,
        orc_context_score AS orc_final_score_v2,
        eligible_v63,
        orc_v7_ready,
        context_ready_now AS context_v2_ready_now,
        v63_reason,
        readiness_reason
      FROM v_orc_candidate_context_best_v1
    $v$;
  ELSE
    EXECUTE $v$
      CREATE VIEW v_learning_orc_context_compat_v1 AS
      SELECT
        symbol, "interval", strategy,
        NULL::numeric AS orc_final_score_v2,
        NULL::boolean AS eligible_v63,
        orc_v7_ready,
        eligible_v7_shadow AS context_v2_ready_now,
        NULL::text AS v63_reason,
        readiness_reason
...
base AS (
  ...
  LEFT JOIN v_market_memory_orc_context_best_v17 mme
    ON mme.symbol = p.symbol
   AND mme."interval" = p."interval"
