...
base AS (
  SELECT
    ...
    et.realtime_json AS entry_realtime_json,
    ...
    xv3.learning_priority AS exit_trace_learning_priority,
    xv3.decision_context AS exit_decision_context,
  ...
  LEFT JOIN LATERAL (
    ...
    WHERE e.symbol = p.symbol
      ...
      AND e.created_at <= p.entry_time + interval '10 minutes'
      AND e.created_at >= p.entry_time - interval '30 minutes'
  ...
  LEFT JOIN v_orc_integration_v2_best orc
  ...
  LEFT JOIN LATERAL (
    ...
    WHERE s.symbol = p.symbol
      AND s."interval" = p."interval"
      AND s.strategy = p.strategy
      AND s.created_at <= p.entry_time + interval '10 minutes'
      AND s.created_at >= p.entry_time - interval '30 minutes'
