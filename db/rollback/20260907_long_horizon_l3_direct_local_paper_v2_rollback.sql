-- Evidence-preserving rollback for LONG HORIZON L3 DIRECT LOCAL PAPER V2.
BEGIN;
...
BEGIN
  IF current_database()<>'trading_paper'
    ...
    RAISE EXCEPTION 'L3_V2_ROLLBACK_LOCAL_PAPER_ONLY';
  END IF;
  IF current_setting('waltrade.target_deployment_id',true) IS DISTINCT FROM 'local-paper' THEN
    RAISE EXCEPTION 'L3_V2_ROLLBACK_LOCAL_PAPER_DEPLOYMENT_REQUIRED';
  END IF;
...
WITH snapshot AS (
  ...
  WHERE c.contract_version='LONG_HORIZON_L3_DIRECT_LOCAL_PAPER_V2'
    AND c.status='ACTIVE'
), restored AS (
  UPDATE public.bot_control b
     SET regime_enabled=s.regime_enabled,
         regime_mode=s.regime_mode,
         updated_at=s.updated_at
    FROM snapshot s
   WHERE b.symbol=s.symbol AND b.interval=s.interval AND b.strategy=s.strategy
     AND (b.regime_enabled,b.regime_mode,b.updated_at)
         IS DISTINCT FROM (s.regime_enabled,s.regime_mode,s.updated_at)
  RETURNING b.symbol
)
...
UPDATE public.long_horizon_l3_contract_v1
   SET status='TERMINATED',
       contract_payload=contract_payload || jsonb_build_object(
         'termination_reason','V2_ROLLBACK_BEFORE_RUNTIME_ACTIVATION',
...
SELECT '20260907_long_horizon_l3_direct_local_paper_v2_rollback',
       ...
       'LONG_HORIZON_L3_DIRECT_LOCAL_PAPER_V2_ROLLBACK'
WHERE NOT EXISTS (
  SELECT 1 FROM public.schema_migration_ledger_v1
   WHERE migration_id='20260907_long_horizon_l3_direct_local_paper_v2_rollback'
