-- Evidence-preserving rollback for LONG HORIZON L3 DIRECT LOCAL PAPER V3.
BEGIN;
...
BEGIN
  IF current_database()<>'trading_paper'
    ...
    RAISE EXCEPTION 'L3_V3_ROLLBACK_LOCAL_PAPER_ONLY';
  END IF;
  IF current_setting('waltrade.target_deployment_id',true) IS DISTINCT FROM 'local-paper' THEN
    RAISE EXCEPTION 'L3_V3_ROLLBACK_LOCAL_PAPER_DEPLOYMENT_REQUIRED';
  END IF;
...
WITH snapshot AS (
  ...
  WHERE c.contract_version='LONG_HORIZON_L3_DIRECT_LOCAL_PAPER_V3'
    AND c.status='ACTIVE'
)
...
UPDATE public.long_horizon_l3_contract_v1
   SET status='TERMINATED',
       contract_payload=contract_payload || jsonb_build_object(
         'termination_reason','V3_ROLLBACK',
...
SELECT '20260908_long_horizon_l3_direct_local_paper_v3_rollback',
       ...
       'LONG_HORIZON_L3_DIRECT_LOCAL_PAPER_V3_ROLLBACK'
WHERE NOT EXISTS (
  SELECT 1 FROM public.schema_migration_ledger_v1
   WHERE migration_id='20260908_long_horizon_l3_direct_local_paper_v3_rollback'
