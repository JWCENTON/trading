...
CONTRACT_VERSION = "LOCAL_LIVE_CLOSED_FT_REPAIR_V1"
PLANNER_VERSION = CONTRACT_VERSION + "_PLANNER"
ENVIRONMENT = "LIVE"
DEPLOYMENT_ID = "local-live"
DATABASE = "trading_live"
POSITION_IDS = tuple(range(3066, 3079))

EXPECTED_EXISTING_EXCLUSION_IDS = frozenset({3077})

EXPECTED_MUTATIONS = (
    "learning_outcome_exclusion_v1:INSERT_IF_ABSENT",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "PLAN-only LOCAL LIVE CLOSED Financial Truth repair V1"
        )
    )
    parser.add_argument(
        "--output",
        default="/tmp/local_live_closed_ft_repair_v1_plan.json",
    )
    parser.add_argument(
        "--git-sha",
        required=True,
    ...
    print("LOCAL_LIVE_CLOSED_FT_REPAIR_PLAN_PASS")
    return 0
