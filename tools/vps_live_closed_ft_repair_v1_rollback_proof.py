...
from tools.vps_live_closed_ft_repair_v1 import (
    CONTRACT_VERSION,
    DATABASE,
    DEPLOYMENT_ID,
    ENVIRONMENT,
...
def insert_or_keep_exclusion(
    ...
    *,
    ...
    if position_id not in EXPECTED_EXISTING_EXCLUSION_IDS:
        raise RuntimeError(
            f"POSITION_OUTSIDE_EXPECTED_EXCLUSION_SET:{position_id}"
        )
...
def verify_artifact_counts(cur):
    ...
    source_identities = [
        f"LIVE:{DEPLOYMENT_ID}:{DATABASE}:position:{position_id}"
    ...
    cur.execute(
        ...
        """,
        (ENVIRONMENT, DEPLOYMENT_ID, position_ids),
    )
    ...
    cur.execute(
        ...
        WHERE incident_type='LEGACY_POSITION'
          AND incident_identity::bigint=ANY(%s)
        """,
        (position_ids,),
    )
                ...
                audit_written = (
                    LegacyRepairAuditRepository.append(
                        cur,
                        {
                            ...
                            "operation_type": (
                                "VPS_LIVE_CLOSED_FT_ONLY_REPAIR"
                            ),
                            "planner_version": PLANNER_VERSION,
